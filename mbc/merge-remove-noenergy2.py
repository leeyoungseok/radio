...
def merge_no_energy_first(df):
    ...
    while i < len(df):
        ...
        if current_row['labels'] == 'noEnergy':
            if i < len(df) - 1:
...
def process_directory(output_dir):
    # Find all directories matching output-yearmonthday pattern
    for root, dirs, files in os.walk(output_dir):
        for dir_name in dirs:
            print("dir_name: ", dir_name, "len:", len(dir_name))
            if dir_name.startswith("output-") and len(dir_name) == 15:  # Check for output-yearmonthday pattern
                #print("dir_name: ", dir_name)
                date = dir_name.split("-")[1]  # Extract yearmonthday part
                input_dir = os.path.join(root, dir_name)
                input_file = os.path.join(input_dir, f"{date}.csv")
                output_file = os.path.join(input_dir, f"{date}-noenergy2.csv")
